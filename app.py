...
def save_entry(ws, name, w1, w2, bias, acc):
    # check if name exists → update, else append
    rows = ws.get_all_records()
...
with st.sidebar:
    ...
    if submit:
        if not name.strip():
            ...
            try:
                ws = get_worksheet()
                save_entry(ws, name.strip(), round(w1,2), round(w2,2), round(bias,2), acc)
...
with col2:
    ...
    try:
        ...
        if not entries:
            ...
            # Sort by accuracy desc, then by submission order
